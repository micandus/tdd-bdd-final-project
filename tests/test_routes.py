...
class TestProductRoutes(TestCase):
    ...
    def test_update_prodcut(self):
        ...
        to_test["description"] = "This is a test"
        response = self.client.put(f'{BASE_URL}/{to_test["id"]}', json=to_test)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        
        #and fetch it pack to prove it was updated
        response = self.client.get(f'{BASE_URL}/{to_test["id"]}')
        to_check = response.get_json()
        self.assertEqual(to_check["description"], "This is a test")

    def test_update_product_not_found(self):
        """ Test 404 exception in updating a product"""
        #create a product in the database first
        to_test = ProductFactory()
        response = self.client.post(BASE_URL, json=to_test.serialize())
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        #then update it with a worng id
